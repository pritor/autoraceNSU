...
class LaneFinder(Node):
    def __init__(self):
        super().__init__('lane_finder')
        self.br = CvBridge()
        # self.subscription = self.create_subscription(Image, '/color/image', self.image_callback, 10)
        # self.publisher_ = self.create_publisher(Image, '/color/image_ch', 10)
        ...
        self.sub_image_original = self.create_subscription(Image, '/color/image', self.cbFindLane, 10)
        ...
        self.pub_image_lane = self.create_publisher(Image, '/detect/image_output', 10)
    ...
    #     ret, thres = cv.threshold(res2, 100, 255, cv.THRESH_BINARY)
    #     countours, hierarchy = cv.findContours(thres, cv.RETR_TREE, cv.CHAIN_APPROX_NONE)
